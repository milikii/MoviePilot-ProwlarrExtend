...
当前 ProwlarrExtend 为 v2.7：修正详情页表头与列不一致，search_torrents 复用统一 headers，补版本契约测试与 CI；v2.6 抽取 mapping 纯函数并补契约测试；v2.5 补全 `TorrentInfo` 站点元数据，虚拟索引器打 parser/plugin 标记并清空蜘蛛路径以减少系统侧空转，增加 `/test` 与 `/status` API。完整历史见 [`package.v2.json`](./package.v2.json)。
