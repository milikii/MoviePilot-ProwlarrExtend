...
| 桥接索引器 | 多选只注册部分 Prowlarr 索引器；**留空=全部**。需先「立即运行一次」拉取列表 |

前置条件：Prowlarr 里的索引器要先配好、能正常搜。剩下的只是这里填地址和 Key 而已。连接测试：插件 API `GET .../ProwlarrExtend/test`。
...
- v2.13 模块化拆分（models/formats/quality/eta/language/codes）；阶段耗时与 failure_code；契约测试
- v2.14 静态检查修正；plugin_version 与 package.v2.json 契约测试；CI
- v2.15 拆分 MediaOps/RuntimeOps/UI mixin，主类瘦身
- v2.16 继续拆分 TranslateOps/Workflow mixin，插件壳约 250 行
- v2.17 抽取 batching/linecheck 纯模块与术语解析
- v2.18 修复 mixin MRO，确保 get_form/get_page/stop_service 可加载

当前 ProwlarrExtend 为 v2.11：搜索超时可配置，失败区分超时/HTTP 错误/空结果并记录条数耗时；v2.10 起多选逻辑在 `selection` 模块；v2.8 起支持桥接索引器多选（空=全部）。完整历史见 [`package.v2.json`](./package.v2.json)。
