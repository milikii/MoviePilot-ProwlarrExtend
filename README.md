...
当前 ProwlarrExtend 为 v2.6：抽取 mapping 纯函数并补契约测试；v2.5 补全 `TorrentInfo` 站点元数据，虚拟索引器打 parser/plugin 标记并清空蜘蛛路径以减少系统侧空转，增加 `/test` 与 `/status` API。完整历史见 [`package.v2.json`](./package.v2.json)。
