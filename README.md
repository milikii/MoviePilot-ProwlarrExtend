...
| 媒体目录或视频路径 | 「立即运行一次」与定时任务共同使用的扫描目标 |
...
当前 SubtitleHunter 为 v2.6：v2.4 引入 AI 自动术语表、行长度后校验、句边界感知分批；v2.5 增加定时执行（cron）和带预计完成时间的启动通知；v2.6 优化通知排版（任务范围、耗时、产物路径、失败原因）。ProwlarrExtend 仍为 v2.3。完整历史见 [`package.v2.json`](./package.v2.json)。
